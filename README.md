...
`sorted 함수의 by 인자는 함수 (내부의 결과로 정렬), 그러므로 해당 클로저 내부에서 두가지 조건을 모두 작성해야 함 -> 수정 완료`
...
`(A->B->A 이런 형태가 아니라면) 굉장히 일반적이며 문제 없음`
...
- 또한 나만 보는 코드가 아니니, Naming을 할 때도 더욱 직관적이고 이해가 쉬운 방향으로 정하기 위한 고민들을 많이 해보았음. <br/>~~(코드 짜는 것 보다 더 어려웠다..)~~
...
프로젝트 이전
- Git, Gitub을 거의 사용해 본 적 없음

프로젝트 이후
- add, commit, push, pull, branch, merge, clone 사용할 수 있음

**[2. Pull requests]**

PR reviewer: 웨더
- Markdown 문법 사용
- 작성한 코드를 글로 설명하는 법 연습
...
처음인 게 참으로 많았다.
첫 협업, 첫 페어 프로그래밍, 첫 Git&Gitub 사용, 첫 PR,  첫 리뷰 반영 수정..

그중 첫 협업에 대한 경험이 가장 깊이 기억에 남을 것 같다.

작은 프로젝트이기 때문에 어쩌면 혼자 구현하는 게 더 편하고, 쉬웠을 지도 모른다.

하지만 함께 머리를 맞대어 고민하고, 수정하고, 서로의 의견을 나누고, 의견을 조율하며

같은 결과를 내지만 수많은 방법이 있다는 걸 깨달았고

내가 생각하지 못한 아이디어를 얻고 배울 수 있는 계기가 되었다.

나의 의견(왜 이 코드가 적절한 지에 대해)을 주장하는 과정에서 생각이 정리되는 것을 느끼고,

앞으로도 누군가에게 설명할 수 있는 정도로 공부, 코드 작성을 해야 한다.

모르는 부분이 참으로 많은 저에게 여러 방향에서 긍정적인 영향을 준


검색왕, 스피드왕 **🦁 Toughie**에게 진한 감사를 전하고 싶다.
